...
def insert_token(token, email,publicKey): #Insert in loggued table
    try:
        result = get_loggued(email)
        if result is False:
            get_db().execute("insert into loggued values(?,?,?)", [email ,token,publicKey])
            get_db().commit()
            return True
        else:
            get_db().execute("update loggued set token = ?, publicKey = ? where email = ?", [token,publicKey,email])
            get_db().commit()
            return True
...
def check_user(email, password):
    try:
        ...
        cursor.close()
        if email == rows[0] and sec.checkPwd(password,rows[1]):
            return True
    except sqlite3.Error as e:
        return False
    except Exception as e:
...
def get_loggued_publicKey(publicKey):
    try:
        cursor = get_db().execute("select * from loggued where publicKey = ?", [publicKey])
        rows = cursor.fetchall()[0]
        cursor.close()
        return {'token' : rows[1]}
    except sqlite3.Error as e:
