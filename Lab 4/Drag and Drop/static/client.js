...
signInVaidator = function(){
  ...
  xhr.onreadystatechange = function(){
    if(this.readyState == 4 && this.status == 200){
      ...
      if(resp.success){
        var newToken = {'username': username, 'token':resp['token']}
...
changePassword = function(){
  ...
  if(oldPassword.localeCompare(newPassword) == 0){
    document.getElementById("changePasswordResultText").innerHTML = "Old password must be different to new password";
    return;
  }
...
userInformation = function(username, token){
  ...
  xhr.open("GET", "/get-data/"+username, true);
  xhr.setRequestHeader('Content-type','application/json; charset=utf-8');
  xhr.setRequestHeader('Authorization', 'Bearer ' + token);
  xhr.send();
}
...
postComment = function(){
  ...
  xhr.setRequestHeader('Content-type','application/json; charset=utf-8');
  xhr.setRequestHeader('Authorization', 'Bearer ' + token);
  xhr.send(JSON.stringify({'message' : comment, 'email' : toEmail}));
}
      ...
      if(resp.success){
        ...
        if(numberMessages > 0){
          for(var commentIndex = numberMessages-1; commentIndex >=0; commentIndex--){
            ...
            commentsHTML += "<div draggable='true' ondragstart='drag(event)' id='" + commentIndex + "'><strong>" + comment['email_sender'] + ":</strong> " + comment['message'] + "</div>";
          }
          if(currentView == "home"){
            document.getElementById("userCommentsWall").innerHTML = commentsHTML;
          }
          else{
            document.getElementById("otherUserCommentsWall").innerHTML = commentsHTML;
          }
        }
      }
    }
  }
  xhr.open("GET", "get-data/message/"+ toemail, true);
  xhr.setRequestHeader('Content-type','application/json; charset=utf-8');
  xhr.setRequestHeader('Authorization', 'Bearer ' + token);
  xhr.send();
}
  ...
  xhr.onreadystatechange = function(){
    if(this.readyState == 4 && this.status){
      ...
      if(resp.success){
        ...
        if(currentView == "home"){
          document.getElementById("resultText").innerHTML = "No such user";
        }else{
          document.getElementById("otherResultText").innerHTML = "No such user";
        }
      }
...
function drag(ev) {
  ev.dataTransfer.setData("text", ev.currentTarget.textContent);  
}
...
function drop(ev) {
  ...
  var message = data[1];
  document.getElementById("dropWall").innerHTML = message;
}
