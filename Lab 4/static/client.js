...
window.onload = function(){
  ...
  if (localStorage.getItem("token") == null){
    ...
    if(userData.length != 0){ //User stukk loggued in
      ...
      userInformation(userData.token);
    }
    else{
      currentView = "welcome";
      usernameFound = false;
...
changePassword = function(){
  ...
  var publicKey = JSON.parse(localStorage.getItem("publicKey")).publicKey;
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function(){
    if(this.readyState == 4 && this.status == 200){
      var resp = JSON.parse(xhr.responseText);
...
signOut = function(){
  ...
  cToken = md5(token);
  xhr.setRequestHeader('Authorization', 'Bearer ' + cToken );
  xhr.send('publicKey':publicKey);
}

userInformation = function(token){
  var publicKey = JSON.parse(localStorage.getItem("publicKey")).publicKey;
  cToken = md5(token);
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function(){
  ...
  xhr.open("GET", "/get-data/"+publicKey, true);
  xhr.setRequestHeader('Content-type','application/json; charset=utf-8');
  xhr.setRequestHeader('Authorization', 'Bearer ' + cToken);
  xhr.send();
}
...
refreshWall = function(){
  ...
  var publicKey = JSON.parse(localStorage.getItem("publicKey")).publicKey;
  cToken = md5(token);
  xhr.open("GET", "get-data/message/"+ publicKey, true);
  xhr.setRequestHeader('Content-type','application/json; charset=utf-8');
  xhr.setRequestHeader('Authorization', 'Bearer ' + cToken);
  xhr.send();
}
